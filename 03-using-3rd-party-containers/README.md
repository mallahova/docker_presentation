...
--interactive --tty - running shell withing that container
--rm once this contatiner process exits, it should not store the container
```bash
...
```
We get different docker container id.
...
docker build --tag my-ubuntu-image examples/dockerfile_1
EOF
...
```
`Dockerfile` is used to specify how to build a container image.
